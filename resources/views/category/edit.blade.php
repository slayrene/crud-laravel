<x-layout>
    ...
    <div class="w-96 bg-white border border-gray-200 rounded-lg p-6 shadow-sm hover:bg-gray-100">
        ...
        <a href ="/categories" class="text-blue-600 underline">Back</a>
        ...
        <form action="{{ url('categories/'.$category->id.'/edit') }}" method="POST">
            ...
            <div class="flex flex-col mt-5 space-y-2">
                ...
                <input type="text" id="name" name="name" class="bg-gray-50 border border-gray-300 rounded w-full" value="{{ $category->name }}">
                @error('name')
                    <span class="text-red-600"> {{ $message }}</span>
                @enderror

                <label for="description">Description</label>
                <textarea name="description" id="description" class="bg-gray-50 border border-gray-300 rounded w-full h-20">{{ $category->description }}</textarea>
                @error('description')
                    <span class="text-red-600"> {{ $message }}</span>
                @enderror
                ...
                <div class="flex items-center space-x-2">
                    ...
                    <input name="is_active" id="is_active" type="checkbox" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded-sm" {{ $category->is_active == true ? 'checked' : '' }}>
                </div>
                @error('is_active')
                    <span class="text-red-600"> {{ $message }}</span>
                @enderror
